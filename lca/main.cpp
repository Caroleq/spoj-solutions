/// Solution to LCA Problem (https://www.spoj.com/problems/LCA/)
...
void parse_tree_from_input(const int nodes_cnt) {
    ...
    for (size_t node_idx = 0; node_idx < nodes_cnt; node_idx++) 
    {
        ...
        for (size_t child_idx = 0; child_idx < child_nodes_cnt; child_idx++) 
        {
            ...
            tree_edges[node_idx].push_back(child_node - 1);
...
int main()
{
    ...
    for (size_t idx = 0; idx < cases_count; idx++)
    {
        ...
        std::cout << "Case " << idx + 1 << ":" << std::endl;
        for (size_t query_idx = 0; query_idx < queries_cnt; query_idx++)
        {
            size_t left, right;
            std::cin >> left;
            std::cin >> right;
            std::cout << lca(left - 1, right - 1) + 1 << std::endl;
        }
    }
